...
final class VersionTests: XCTestCase {
  func testCompareVersionsHandlesBasicSemver() {
    XCTAssertEqual(compareVersions("0.1.27", "0.1.27"), 0)
    XCTAssertGreaterThan(compareVersions("0.1.28", "0.1.27"), 0)
    XCTAssertLessThan(compareVersions("0.1.9", "0.1.10"), 0)
    XCTAssertEqual(compareVersions("v0.1.27", "0.1.27"), 0)
  }

  func testRuntimeHealthDecodesFromRPCPayload() throws {
    let data = Data("""
    {"status":"ok","version":"0.1.27","uptimeSeconds":12,"pid":42}
    """.utf8)
    let health = try JSONDecoder().decode(RuntimeHealth.self, from: data)

    XCTAssertEqual(health.status, "ok")
    XCTAssertEqual(health.version, "0.1.27")
