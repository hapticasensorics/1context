...
private let memoryWikiUpdateInitialDelay: TimeInterval = 0
private let memoryWikiUpdateTimerTickInterval: TimeInterval = 60
...
private let memoryWikiUpdateTimeoutSeconds = 20 * 60
...
final class OneContextDaemon: @unchecked Sendable {
  ...
  private var memoryWikiUpdateLastAutomaticStartedAt: Date?
  ...
  private func contextEngineUpdateWikiPayload(params: [String: Any]) -> [String: Any] {
    ...
    let sourceWindowDays = intParam(params["source_window_days"]) ?? 3
    let configuredAgentLimit = OneContextAppSettings
      .wikiAgentConcurrencyLimit(preferencesPath: paths.preferencesPath)
      .contextEngineArgumentValue
    let maxConcurrent = intParam(params["max_concurrent"]) ?? configuredAgentLimit
    ...
    do {
      ...
      payload["context_engine"] = result
      finalStatus = contextEngineOperationStatus(result)
      payload["context_engine_status"] = finalStatus == "failed" ? "failed" : "ok"
      if finalStatus == "failed" {
        finalError = "context-engine update-wiki returned failed"
      }
    } catch {
      logger.write("context_engine.update_wiki unavailable: \(error.localizedDescription)")
      payload["context_engine_status"] = "unavailable"
      payload["context_engine_error"] = error.localizedDescription
      finalStatus = "failed"
    ...
    payload["memory_update"] = memoryWikiUpdateStatusPayload()
    logger.write(
      "context_engine.update_wiki completed trigger=\(trigger) status=\(finalStatus) run_id=\(runID) orchestrator=onecontext-context-engine"
    )
    return payload
  }
  ...
  private func startAutomaticMemoryWikiUpdateTimer() {
    memoryWikiUpdateQueue.async { [weak self] in
      ...
      timer.schedule(
        ...
        repeating: memoryWikiUpdateTimerTickInterval,
        leeway: .seconds(5)
      )
      timer.setEventHandler { [weak self] in
      ...
      timer.resume()
      self.logger.write(
        "automatic context_engine.update_wiki timer started tick_interval=\(Int(memoryWikiUpdateTimerTickInterval))s initial_delay=\(Int(memoryWikiUpdateInitialDelay))s cursor=\(memoryWikiUpdateCursorName)"
      )
    }
  }

  private func performScheduledMemoryWikiUpdate() {
    let cadence = OneContextAppSettings.wikiAutomaticUpdateCadence(preferencesPath: paths.preferencesPath)
    guard cadence.isEnabled else {
      return
    }
    let remaining = automaticMemoryWikiUpdateCadenceRemaining(cadence: cadence, at: Date())
    ...
    markAutomaticMemoryWikiUpdateStarted()

    let sourceWindowDays = firstProcessPass ? memoryWikiBackfillWindowDays : memoryWikiIncrementalWindowDays
    let agentLimit = OneContextAppSettings.wikiAgentConcurrencyLimit(preferencesPath: paths.preferencesPath)
    ...
    do {
      let result = try performContextEngineWikiUpdate(
        ...
        runID: runID,
        executeAgents: true,
        maxConcurrent: agentLimit.contextEngineArgumentValue,
        sourceWindowDays: sourceWindowDays,
        timeoutSeconds: memoryWikiUpdateTimeoutSeconds
      )
      let status = contextEngineOperationStatus(result)
      finishMemoryWikiUpdate(status: status, error: status == "failed" ? "context-engine update-wiki returned failed" : nil)
      publishWikiInBackground(refresh: false)
      logger.write(
        "automatic context_engine.update_wiki completed trigger=\(trigger) status=\(status) run_id=\(runID) cadence=\(cadence.rawValue) agent_limit=\(agentLimit.rawValue) orchestrator=onecontext-context-engine"
      )
    } catch {
      finishMemoryWikiUpdate(status: "failed", error: error.localizedDescription)
      logger.write("automatic context_engine.update_wiki failed trigger=\(trigger): \(error.localizedDescription)")
    }
  }

  private func automaticMemoryWikiUpdateCadenceRemaining(
    cadence: WikiAutomaticUpdateCadence,
    at date: Date
  ) -> TimeInterval {
    guard let interval = cadence.minimumAutomaticInterval,
      interval > 0
    else {
      return 0
    }
    memoryWikiUpdateStateLock.lock()
    let lastAutomaticStartedAt = memoryWikiUpdateLastAutomaticStartedAt
    memoryWikiUpdateStateLock.unlock()
    guard let lastAutomaticStartedAt else {
      return 0
    }
  ...
  private func contextEngineOperationStatus(_ payload: [String: Any]) -> String {
    ...
    if let marker = payload["wiki_refresh_marker"] as? [String: Any],
      let markerStatus = marker["status"] as? String,
      markerStatus == "failed" || markerStatus == "unavailable"
    {
      return "failed"
    }
    let status = (payload["status"] as? String) ?? ""
    if status == "completed" || status == "drafted" || status == "written" || status == "planned" || status == "executed" {
      return "completed"
    }
    if status == "ok" {
      return "completed"
  ...
  private func markAutomaticMemoryWikiUpdateStarted() {
    memoryWikiUpdateStateLock.lock()
    defer { memoryWikiUpdateStateLock.unlock() }
    memoryWikiUpdateLastAutomaticStartedAt = memoryWikiUpdateLastStartedAt ?? Date()
  }
  ...
  private func memoryWikiUpdateStatusPayload() -> [String: Any] {
    let automaticUpdateCadence = OneContextAppSettings.wikiAutomaticUpdateCadence(preferencesPath: paths.preferencesPath)
    let agentLimit = OneContextAppSettings.wikiAgentConcurrencyLimit(preferencesPath: paths.preferencesPath)
    let automaticCadenceRemainingMilliseconds = Int(
      (automaticMemoryWikiUpdateCadenceRemaining(cadence: automaticUpdateCadence, at: Date()) * 1_000).rounded()
    )
    memoryWikiUpdateStateLock.lock()
    defer { memoryWikiUpdateStateLock.unlock() }
    let lastAutomaticStartedAt = memoryWikiUpdateLastAutomaticStartedAt
    var payload: [String: Any] = [
      ...
      "automatic_cadence": automaticUpdateCadence.rawValue,
      "automatic_cadence_label": automaticUpdateCadence.title,
      "automatic_enabled": automaticUpdateCadence.isEnabled,
      "automatic_interval_seconds": Int(automaticUpdateCadence.minimumAutomaticInterval ?? 0),
      "automatic_timer_tick_seconds": Int(memoryWikiUpdateTimerTickInterval),
      "automatic_initial_delay_seconds": Int(memoryWikiUpdateInitialDelay),
      "automatic_cadence_remaining_ms": automaticCadenceRemainingMilliseconds,
      "agent_concurrency_limit": agentLimit.rawValue,
      "agent_concurrency_limit_label": agentLimit.title,
      "max_concurrent_agents": agentLimit.contextEngineArgumentValue,
    ...
    if let lastAutomaticStartedAt {
      payload["last_automatic_started_at"] = ISO8601DateFormatter().string(from: lastAutomaticStartedAt)
    }
