...
talk routing, mailbox delivery, agent directory, notification, sitemap, or
...
- `wiki_ensure`, `wiki_list`, `wiki_page_status`, `wiki_page_open`, `wiki_page_create`,
  ...
  `wiki_page_restore`, `wiki_publish_status`, `wiki_publish`, `wiki_agent_identify`,
...
6. Use `wiki_talk_append` with inline `body_markdown` for short notes or
   `body_file` for prepared markdown. The adapter requires exactly one body
   source, matching page body writes.
7. Treat talk, inbox, notifications, claims, and mail state changes as
...
8. Recheck `wiki_page_status` or `wiki_list` after publish/delete/restore.
...
- talk delivery, inbox state, or notifications
