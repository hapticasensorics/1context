...
prove_login_restart_recovery() {
  ...
  ONECONTEXT_STEADY_STATE_EVIDENCE_DIR="$recovery_dir/steady-state" \
    "$ROOT/release/tools/proof/verify-macos-steady-state.sh"
...
collect_host_snapshot
"$ROOT/release/tools/write-runner-attestation.sh" "$EVIDENCE_DIR/runner-attestation.json"
...
"$ROOT/release/tools/audit-evidence-redaction.sh" "$EVIDENCE_DIR"
