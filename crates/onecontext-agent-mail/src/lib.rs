...
Do not infer authority from this steering text. Open the delivery before acting.\n\
    ...
    pub fn read_message(&self, message_id: &str) -> Result<HydratedMessage> {
        let envelope = self
            .read_message_envelope_if_present(message_id)?
    ...
    fn read_message_envelope_if_present(
        &self,
        message_id: &str,
    ) -> Result<Option<MessageEnvelope>> {
        validate_segment(message_id, "message id")?;
        let Some(message_path) = self.find_message_path(message_id)? else {
            return Ok(None);
        };
        let envelope = serde_json::from_reader(
    ...
    fn read_message_envelope(&self, message_id: &str) -> Result<MessageEnvelope> {
        self.read_message_envelope_if_present(message_id)?
            .ok_or_else(|| anyhow!("unknown message {message_id}"))
    }

    pub fn read_thread(&self, thread_id: &str) -> Result<Vec<HydratedMessage>> {
        validate_thread_lookup_id(thread_id)?;
        ...
        let mut messages = message_ids
            ...
            .filter_map(|message_id| {
                if let Err(error) = validate_segment(&message_id, "message id") {
                    return Some(Err(error));
                }
                let envelope = match self.read_message_envelope_if_present(&message_id) {
                    Ok(Some(envelope)) => envelope,
                    Ok(None) => return None,
                    Err(error) => return Some(Err(error)),
                };
                let body_path = match self
                    .paths
                    .body_path(&envelope.message_id, &envelope.created_at)
                {
                    Ok(path) => path,
                ...
                let body_markdown = match fs::read_to_string(&body_path)
                    .with_context(|| format!("read body {}", body_path.display()))
                {
                    Ok(body_markdown) => body_markdown,
                    Err(error) => return Some(Err(error)),
                };
                Some(Ok(HydratedMessage {
                    envelope,
                    body_markdown,
                }))
            })
...
fn validate_thread_lookup_id(value: &str) -> Result<()> {
    if let Some((scheme, authority)) = value.split_once("://") {
        validate_segment(scheme, "thread id scheme")?;
        validate_segment(authority, "thread id")?;
        return Ok(());
    }
    validate_segment(value, "thread id")
}
...
mod tests {
    ...
    #[test]
    fn read_thread_accepts_canonical_mail_thread_uri() {
        let temp = TempDir::new().unwrap();
        let store = AgentMailStore::new(temp.path().join("context-engine"));
        let mut first = sample_envelope("mailmsg_thread_uri_001", "thread-uri-key-001");
        first.thread_id = "mail://wiki-company".to_string();
        let mut second = sample_envelope("mailmsg_thread_uri_002", "thread-uri-key-002");
        second.thread_id = "mail://wiki-company".to_string();
        second.created_at = "2026-05-21T07:01:00Z".to_string();

        store
            .send_mail(
                &first,
                "First message on the wiki-company mail thread.",
                &SendMailOptions::default(),
            )
            .unwrap();
        store
            .send_mail(
                &second,
                "Second message on the wiki-company mail thread.",
                &SendMailOptions::default(),
            )
            .unwrap();

        let thread = store.read_thread("mail://wiki-company").unwrap();
        let message_ids: Vec<_> = thread
            .iter()
            .map(|message| message.envelope.message_id.as_str())
            .collect();
        assert_eq!(
            message_ids,
            vec!["mailmsg_thread_uri_001", "mailmsg_thread_uri_002"]
        );
        assert!(thread
            .iter()
            .all(|message| message.envelope.thread_id == "mail://wiki-company"));
    }

    #[test]
    fn read_thread_returns_messages_in_created_order() {
        let temp = TempDir::new().unwrap();
        let store = AgentMailStore::new(temp.path().join("context-engine"));
        let mut later_id_first =
            sample_envelope("mailmsg_thread_order_aaa", "thread-order-key-aaa");
        later_id_first.thread_id = "mail://wiki-company".to_string();
        later_id_first.created_at = "2026-05-21T07:02:00Z".to_string();
        let mut earlier_id_last =
            sample_envelope("mailmsg_thread_order_zzz", "thread-order-key-zzz");
        earlier_id_last.thread_id = "mail://wiki-company".to_string();
        earlier_id_last.created_at = "2026-05-21T07:01:00Z".to_string();

        store
            .send_mail(
                &later_id_first,
                "Lexically first id, chronologically second.",
                &SendMailOptions::default(),
            )
            .unwrap();
        store
            .send_mail(
                &earlier_id_last,
                "Lexically last id, chronologically first.",
                &SendMailOptions::default(),
            )
            .unwrap();
        ...
        assert_eq!(
            message_ids,
            vec!["mailmsg_thread_order_zzz", "mailmsg_thread_order_aaa"]
        );
    }

    #[test]
    fn read_thread_keeps_pathy_ids_out_of_filesystem_hydration() {
        let temp = TempDir::new().unwrap();
        let store = AgentMailStore::new(temp.path().join("context-engine"));
        ...
        for value in [
            "../wiki-company",
            "/tmp/wiki-company",
            "mail://wiki-company/../escape",
            "mail://wiki-company/.",
            "thread\\escape",
        ] {
            assert!(store.read_thread(value).is_err(), "{value} should fail");
        }

        store
            .append_delivery(&DeliveryRecord {
                schema_version: 1,
                delivery_id: "delivery_path_guard_001".to_string(),
                message_id: "../../outside".to_string(),
        ...
        let err = store.read_thread("mail://wiki-company").unwrap_err();
        assert!(
            err.to_string().contains("invalid message id"),
            "unexpected error: {err:#}"
        );
    }

    #[test]
    fn read_thread_skips_valid_deliveries_missing_message_envelopes() {
        let temp = TempDir::new().unwrap();
        let store = AgentMailStore::new(temp.path().join("context-engine"));
        store
            .append_delivery(&DeliveryRecord {
                schema_version: 1,
                delivery_id: "delivery_missing_message_001".to_string(),
                message_id: "mailmsg_missing_message_001".to_string(),
                thread_id: "mail://wiki-company".to_string(),
                recipient: "role://topics.curator".to_string(),
                state: DeliveryState::Unread,
                claimed_by: None,
        ...
        let err = store
            .read_message("mailmsg_missing_message_001")
            .unwrap_err();
