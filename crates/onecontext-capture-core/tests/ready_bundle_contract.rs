...
#[test]
fn export_copies_2fps_frames_into_bundle_media() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    let now = seed_window_snapshot(&capture_root);
    ...
    let manifest_text = decoded_frame_manifest_text(now);
    fs::write(frame_dir.join("frames-2fps-manifest.jsonl"), &manifest_text).unwrap();
    let response = export_ready_bundle(ExportRequest {
        capture_root,
        time_start: now - Duration::seconds(1),
        time_end: now + Duration::seconds(1),
    ...
    assert!(response
        .bundle_path
        .join("media/frames-2fps/frames-2fps-manifest.jsonl")
        .exists());
    assert_eq!(
        fs::read_to_string(
            response
    ...
    assert_frame_timing_record(
        &media,
        "frame-a.jpg",
        0.0,
        0.016,
        16.0,
        (now - Duration::milliseconds(484)).to_rfc3339(),
    );
    assert_frame_timing_record(&media, "frame-b.png", 0.5, 0.5, 0.0, now.to_rfc3339());
...
#[test]
fn export_rejects_debug_video_as_ready_bundle_media() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    let now = seed_window_snapshot(&capture_root);
    ...
    let error = export_ready_bundle(ExportRequest {
        capture_root,
        time_start: now - Duration::seconds(1),
        time_end: now + Duration::seconds(1),
        target: CaptureTarget::ActiveWindow,
    ...
    assert!(error
        .to_string()
        .contains("debug video is not READY bundle media"));
}

#[test]
fn export_omits_debug_video_when_not_requested_but_keeps_decoded_frame_timing() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    let now = seed_window_snapshot(&capture_root);
    let frame_dir = temp.path().join("decoder-frames");
    ...
    fs::write(frame_dir.join("frame-b.png"), "png-b").unwrap();
    fs::write(
        frame_dir.join("frames-2fps-manifest.jsonl"),
        decoded_frame_manifest_text(now),
    )
    .unwrap();
    let unused_debug_video = temp.path().join("screen-recording.mov");
    fs::write(&unused_debug_video, "debug movie").unwrap();
    ...
    assert_eq!(response.state, "ready");
    assert!(!response.bundle_path.join("media/debug").exists());
    ...
    assert!(!media
        .iter()
        .any(|record| record["kind"] == "debug_screen_recording"));
    assert_frame_timing_record(
        &media,
        "frame-a.jpg",
        0.0,
...
#[test]
fn window_snapshot_display_payload_does_not_backfill_display_lane() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    ...
    assert!(
        displays.is_empty(),
        "window display payloads should remain window evidence, not manufactured display lane records"
    );
    ...
    let displays_source = sources
        ...
        .unwrap();
    assert_eq!(displays_source.status, SourceStatus::Degraded);
    assert_eq!(displays_source.record_count, 0);
}
...
#[test]
fn ux_and_sampler_capabilities_are_derived_from_capture_status() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    let now = seed_window_snapshot(&capture_root);
    ...
    let response = export_ready_bundle(ExportRequest {
        ...
        status_json: Some(json!({
            ...
            "ux_event_tap": {
                ...
                "observed_event_count": 123
            },
            "continuous_metadata_sampler": {
                "enabled": true,
                "last_error": null,
                "sample_count": 4
            }
    ...
    let ux_status: Value = read_json(&response.bundle_path.join("capabilities/ux-event-tap.json"));
    let samplers: Value = read_json(&response.bundle_path.join("capabilities/samplers.json"));
    assert_eq!(ux_status["status"], "ok");
    assert_eq!(ux_status["source"], "capture.status.ux_event_tap");
    assert_eq!(ux_status["ux_event_tap"]["observed_event_count"], 123);
    assert_eq!(samplers["status"], "ok");
    assert_eq!(
        samplers["source"],
        "capture.status.continuous_metadata_sampler"
    );
    assert_eq!(samplers["continuous_metadata_sampler"]["sample_count"], 4);
...
#[test]
fn connector_lanes_are_not_inferred_from_ax_window_or_sck_evidence() {
    let temp = tempdir().unwrap();
    let capture_root = temp.path().join("capture");
    fs::create_dir_all(capture_root.join("windows")).unwrap();
    ...
    for lane_file in [
        ...
        assert!(records.is_empty(), "{lane_file}");
    }
...
fn seed_window_snapshot_with_display_mode(
    ...
    fs::create_dir_all(capture_root.join("windows")).unwrap();
    let now = Utc::now();
    let window_line = json!({
        "schemaVersion": 1,
        "eventType": "capture.window_snapshot",
        "recordedAt": now.to_rfc3339(),
        "payload": {"windows": [{"windowID": 7}]}
    });
    let mut lines = vec![window_line.to_string()];
    if include_displays {
        lines.push(
            json!({
                "schemaVersion": 1,
...
fn decoded_frame_manifest_text(now: chrono::DateTime<Utc>) -> String {
    format!(
        "{}\n{}\n",
        json!({
            "schema_version": 1,
            "frame_index": 1,
            "file_name": "frame-a.jpg",
            "path": "frames-2fps/frame-a.jpg",
            "requested_media_time_seconds": 0.0,
            "actual_media_time_seconds": 0.016,
            "requested_at": (now - Duration::milliseconds(500)).to_rfc3339(),
            "recorded_at": (now - Duration::milliseconds(484)).to_rfc3339(),
            "requested_to_actual_delta_milliseconds": 16.0,
            "requested_time_tolerance_before_seconds": 0.25,
            "requested_time_tolerance_after_seconds": 0.25,
            "media_zero_time": (now - Duration::milliseconds(500)).to_rfc3339(),
            "decoder_source": "screen_recording.mov",
            "decoder_implementation": "AVAssetImageGenerator"
        }),
        json!({
            "schema_version": 1,
            "frame_index": 2,
            "file_name": "frame-b.png",
            "path": "frames-2fps/frame-b.png",
            "requested_media_time_seconds": 0.5,
            "actual_media_time_seconds": 0.5,
            "requested_at": now.to_rfc3339(),
            "recorded_at": now.to_rfc3339(),
            "requested_to_actual_delta_milliseconds": 0.0,
            "requested_time_tolerance_before_seconds": 0.25,
            "requested_time_tolerance_after_seconds": 0.25,
            "media_zero_time": (now - Duration::milliseconds(500)).to_rfc3339(),
            "decoder_source": "screen_recording.mov",
...
fn assert_frame_timing_record(
    media: &[Value],
    original_file_name: &str,
    requested_media_time_seconds: f64,
    actual_media_time_seconds: f64,
    requested_to_actual_delta_milliseconds: f64,
    recorded_at: String,
) {
    assert!(media.iter().any(|record| {
        record["kind"] == "frame_2fps"
            && record["original_file_name"] == original_file_name
            && record["timing_source"] == "screen_recording_decoder_manifest"
            && record["timing_precision"] == "actual_decoded_media_time"
            && record["requested_media_time_seconds"] == json!(requested_media_time_seconds)
            && record["actual_media_time_seconds"] == json!(actual_media_time_seconds)
            && record["requested_to_actual_delta_milliseconds"]
                == json!(requested_to_actual_delta_milliseconds)
            && record["recorded_at"] == json!(recorded_at)
            && record["frame_timing_manifest_ref"] == "media/frames-2fps/frames-2fps-manifest.jsonl"
            && record["decoder_source"] == "screen_recording.mov"
            && record["decoder_implementation"] == "AVAssetImageGenerator"
    }));
}
