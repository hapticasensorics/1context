...
pub use supervisor::{
    ...
    ensure_managed_postgres_ready_with_runner_and_bootstrapper, stop_managed_postgres_with_config,
    ManagedPostgresCommandRunner, ManagedPostgresEnsureOptions, ManagedPostgresReadyState,
    ManagedPostgresSchemaBootstrapSummary, ManagedPostgresSchemaBootstrapper,
    StdManagedPostgresCommandRunner, StdManagedPostgresSchemaBootstrapper,
};
...
mod tests {
    ...
    fn managed_paths_follow_application_support_contract() {
        ...
        assert_eq!(
            ...
            paths.bootstrap_dir.join("initialized.ok")
        );
        assert_eq!(
            paths.config_fingerprint,
            paths.bootstrap_dir.join("config.sha256")
        );
    }
