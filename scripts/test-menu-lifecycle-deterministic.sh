...
assert_contains "runAppUpdateFlow" "$MENU_SOURCE"
assert_contains "AppUpdateSnapshot" "$MENU_SOURCE"
...
assert_not_contains "isPausingRuntimeForMandatoryUpdate" "$MENU_SOURCE"
...
assert_contains "LaunchAgentManager().startMenu(appPath: appPath)" "$MENU_SOURCE"
...
assert_contains "startSetupReadinessObservation(message:" "$MENU_SOURCE"
assert_contains "message ?? \"Local Wiki Access is ready.\"" "$MENU_SOURCE"
assert_contains "snapshot.shippedPermissionRows" "$MENU_SOURCE"
assert_contains "pollSetupReadiness()" "$MENU_SOURCE"
assert_contains "setupWindowController.window?.isVisible == true" "$MENU_SOURCE"
assert_contains "setRefreshButtonVisible(!snapshot.localWikiAccess.ready || isGrantingLocalWikiAccess)" "$MENU_SOURCE"
assert_contains "removeArrangedSubview(refreshButton)" "$MENU_SOURCE"
