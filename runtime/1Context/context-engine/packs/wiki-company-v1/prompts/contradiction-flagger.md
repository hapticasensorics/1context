...
For the demo product, prefer aggressive cleanup recommendations.
When an older claim is stale, misleading, or contradicted, say
plainly whether the curator should **delete** it or **replace** it.
Do not recommend keeping visible stale caveats unless the historical
sequence itself is important.
...
**Recommended curator action**:

Delete or replace the older claim with `<current wording>`.
